...
class pms_projects(models.Model):
    ...
    project_duration_from_invoices = fields.Integer(compute="_compute_project_duration_from_invoices", string="Duration from Invoices", store=True, help="Project duration automatically extracted from invoice reports")
    ...
    delayed_invoice_payments = fields.Integer(
        string="Delayed Invoice Payments",
        store=True,
        help="Number of invoices that were paid 3+ days after due date"
    )
    delayed_invoice_list = fields.Many2many(
        ...
        string="Delayed Invoice Payments",
        store=True,
        help="List of invoices that were paid 3+ days after due date"
    )
    ...
    total_late_days = fields.Integer(
        ...
        compute="_compute_total_late_days",
        store=True,
...
#Funcion que chequea si las ultima actividad fue completada NO COMPLETADA
    ...
    def _compute_delayed_invoice_payments(self):
        ...
        for record in self:
            ...
            invoice_lines = self.env["account.analytic.line"].search([
                ("account_id", "=", record.address.analytical_account.id), 
                ("category", "=", "invoice")
            ]).mapped("move_line_id")
            ...
            for invoice in invoices:
                if not invoice.date_paid or not invoice.invoice_date_due:
                    continue
    ...
    def _calculate_invoice_delay_days(self, invoice, today):
        if not invoice.invoice_date_due or not invoice.date_paid:
            return 0
    ...
    @api.depends("start_project", "end_project", "project_duration_from_invoices")
    def _calculate_project_duration(self):
        for record in self:
            if record.project_duration_from_invoices:
                record.project_duration = record.project_duration_from_invoices
            elif record.start_project and record.end_project: 
    ...
    @api.depends('address', 'address.analytical_account')
    def _compute_project_duration_from_invoices(self):
        for record in self:
            if not record.address or not record.address.analytical_account:
                record.project_duration_from_invoices = 0
                continue
            ...
            invoices = invoice_lines.mapped("move_id").filtered(lambda inv: inv.move_type == 'out_invoice' and inv.invoice_date)
            
            if not invoices:
                record.project_duration_from_invoices = 0
                continue
            
            sorted_invoices = invoices.sorted(key=lambda inv: inv.invoice_date)
            first_invoice_date = sorted_invoices[0].invoice_date
            last_invoice_date = sorted_invoices[-1].invoice_date
            
            duration_days = (last_invoice_date - first_invoice_date).days
            record.project_duration_from_invoices = max(0, duration_days)
...
#Function that checks if a property is already linked to a project
    ...
    def action_recompute_invoice_fields(self):
        ...
        return {
            ...
            'params': {
                'title': 'Delayed Invoices Recalculated',
                'message': f'Found {self.delayed_invoice_payments} invoice(s) that were paid 3+ days late.',
                'sticky': False,
                'type': 'success',
            }
        }
    
    def action_recompute_project_duration(self):
        self._compute_project_duration_from_invoices()
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': 'Project Duration Updated',
                'message': f'Project duration has been recalculated from invoice reports: {self.project_duration_from_invoices} days',
                'sticky': False,
                'type': 'success',
            }
        }
...
class pms_projects_update_house_model(models.TransientModel):
    ...
    def _send_expiration_date_reminders(self):
        ...
        that is due in 30 days (permits expiration warning).
        """
        _logger.info("Running scheduled action: _send_expiration_date_reminders")
        
        # Calculate the target date (30 days from today)
        target_date = fields.Date.today() + timedelta(days=30)
        
        # Find projects with expiration_date equal to 30 days from now
