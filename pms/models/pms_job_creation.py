...
class PmsJobCreation(models.Model):
    ...
    migrated_to_contractor_job = fields.Many2one('pms.contractor.job', string='Migrated To', readonly=True)
    ...
    def action_migrate_to_contractor_job(self):
        ContractorJob = self.env['pms.contractor.job']
        migrated = []
        for record in self:
            if record.migrated_to_contractor_job:
                continue
            ...
            vals = {
                'property_id': record.address.id if record.address else False,
                'name': record.name.job_name if record.name else 'Migrated Job',
                'description': record.description,
                'deadline': record.due_date,
                'contractor_id': record.contractor.id if record.contractor else False,
                'state': state_map.get(record.status, 'created'),
                'legacy_job_creation_id': record.id,
                'maintenance_order': not record.invoiced,
            }

            if record.activity_id:
                vals['linked_project_activity'] = record.activity_id.id

            job = ContractorJob.create(vals)

            if record.invoice_id:
                invoice = self.env['account.move'].browse(record.invoice_id)
                if invoice.exists():
                    job.linked_invoice = invoice.id

            record.migrated_to_contractor_job = job.id
            migrated.append(job.id)

        if migrated:
            return {
                'type': 'ir.actions.act_window',
                'name': 'Migrated Contractor Jobs',
                'res_model': 'pms.contractor.job',
                'view_mode': 'list,form',
                'domain': [('id', 'in', migrated)],
                'target': 'current',
            }

    @api.model
    def action_migrate_all_to_contractor_jobs(self):
        pending = self.search([('migrated_to_contractor_job', '=', False)])
        if pending:
            return pending.action_migrate_to_contractor_job()
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': 'Migration Complete',
                'message': 'All job creation records have already been migrated.',
                'sticky': False,
            }
        }
