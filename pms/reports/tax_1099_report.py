        ...
        query = f"""
                    ...
                    inner join (
                        ...
                        where (((account_account.account_type = 'liability_credit_card' or account_account.account_type = 'asset_cash' or (account_account.name->>'en_US') ilike '%%outstanding%%' or
                          account_account.code_store = '200404' or (account_account.account_type = 'liability_payable' and account_move_line.matching_number is not null))
                          and account_move_line.credit > 0) or (account_account.code_store = '6.3' and account_move_line.debit > 0)) and account_move_line.parent_state = 'posted'
                         ) paml on aml.move_id = paml.paid_move_id

                    where res_partner.box_1099_id is not null and (account_account.account_type IN ('expense', 'expense_depreciation', 'expense_direct_cost') or account_account.code_store = '121111' or account_account.code_store = '1815202')
