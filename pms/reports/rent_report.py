...
class pms_rent_report(models.Model):
    ...
    arv = fields.Float(string="ARV") # Determined by property (pms.property)
    ...
    improvement = fields.Float(string='Home Improvement', readonly=True)
    ...
    equity_refinance = fields.Float(
        ...
        help="Equity = (ARV * 67%) - Loan Balance"
    )
    ...
    def _table_query(self):
        ...
        WITH PropertyUnitCounts AS (
            ...
            SELECT
                ...
                END AS utilities,
                CASE
                    WHEN MAX(puc.unit_count) > 0 THEN MAX(aal.improvement) / MAX(puc.unit_count)
                    ELSE MAX(aal.improvement)
                END AS improvement,
                CASE
                ...
                END AS mortgage,
                CASE
                    WHEN MAX(puc.unit_count) > 0 THEN MAX(pp.arv) / MAX(puc.unit_count)
                    ELSE MAX(pp.arv)
                END AS arv
            ...
            LEFT JOIN (
                SELECT
                    aal.x_plan23_id,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Repairs%%') AS repairs,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Utilities%%') AS utilities,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Taxes%%') AS taxes,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Insurance%%') AS insurance,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%HOA%%') AS hoa,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Services%%') AS services,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Home Improvement%%') AS improvement,
                    SUM(aml.balance) FILTER (WHERE aat.name->>'en_US' ILIKE '%%Turnover%%') AS turnover
                ...
                WHERE aal.date BETWEEN '{start_date}' AND '{end_date}'
                    AND aat.name->>'en_US' IN ('Repairs', 'Utilities', 'Taxes', 'Insurance', 'HOA', 'Services', 'Turnover', 'Home Improvement')
                GROUP BY aal.x_plan23_id
            ) aal ON pp.analytical_account = aal.x_plan23_id
            LEFT JOIN (
                SELECT
                    aal.x_plan23_id as account_id,
                ...
                INNER JOIN account_account_tag aat ON tagrel.account_account_tag_id = aat.id
                WHERE aal.date BETWEEN '{start_date}' AND '{end_date}'
                    AND aat.name->>'en_US' ILIKE '%%Mortgage%%'
                GROUP BY aal.x_plan23_id
